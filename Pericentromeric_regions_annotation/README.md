...
Since the definition of pericentromeric regions highly depends on the context considered, I also produced another, more restrictive, definition of these PICS based on PICS-specific cut off appplied on alignment scores of these sequences [Figure 8](figures/Fig8.png). The latest selection of PICS will be the one used for the definition of pericentromeric regions. 

## Selecting satellite sequences of interest.
After defining PICS I also selected some satellite sequences in order to define those who could be pericentromeric. Based on the litterature introduced before, I selected alpha, beta and HSATII satellites. As these repeats are already annotated on hg19, I retrieved there coordinates from the UCSC genome browser databases. After assessing their distribution throughout hg19, I was able to confirm they were more densely annotated when closer to centromeres. To restrict to satellites closest to centromeres I decided to remove beta satellites from the selection afterward. Even though the density of beta satellites increase near centromeres, their position on chromosomes seemed more distant from centromeres than alpha satellites and HSATII sequences (see [Figure 9](figures/Fig9.png)). on most hg19 chromosomes I found beta satellites to be framing the most centromeric alpha satellites and HSATII sequences, probably defining another type of genomic regions farther than pericentromeres.
Then, I decided to set a cut-off on their density in the genome. For each chromosome, I considered as pericentromeric, alpha satellites and HSATII sequences:  
...
Using these density cut-off, I have been able to retain many sattelites considered initially (see [Figure 10](figures/Fig10.png)): unexpectedly, almost all HSATII satellite annotations were kept after applying the cut-off. Therefore, here HSATII satellites are considered more pericentromeric than alpha satellites.  
...
Based on all the selected PICS and satellites (see [Figure 11](figures/Fig11.png)) I defined pericentromeric regions as following for each chromosome:  
...
The pericentromeric regions defined this way are sumarized on [Figure 12](figures/Fig12.png).  
Pericentromeric regions have been defined on almost all chromosome, except 2: chromosome 4 and 5.  
On chromosome 4:  
...
On chromosome 5:
* Aligned PICS did not has a sufficient alignment score to pass the PICS-specific cut-offs. None were retained.
* Regarding satellites, the only 2 beta satellites annotated on it were removed. Among the remaining alpha satellites some were closely distributed beyond the proximal half of the chromosome p arm. Thus, the selection based on their density failed, in a similar way than in chromosome 4.
